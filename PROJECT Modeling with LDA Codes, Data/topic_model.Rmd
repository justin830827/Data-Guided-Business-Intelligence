...
```{r}
wp <- apply(exp(LDA_model@beta), 2, max)
top100<- sort(wp,decreasing = T,index.return=T)
...
```{r}
plot(1:100,top100_vals,"o",xlab = "word",ylab = "Max Probabiliy")
...
for( k in seq(20,200,20)){
  ...
  LDA_model <- c(LDA_model,model)
}
perplex <- sapply(LDA_model, function(x) perplexity(x,test))
...
title(xlab = "Number of topic", ylab = "Perplexity", main = "Growth")
```
...
NPMI <- sapply(LDA_model, function(LDA_model) compute_c(LDA_model@beta,test,15,"NPMI"))
NPMI <- data.frame(seq(10,100,10),NPMI)
#plot
plot(NPMI,type = "o", col= "blue",xlab = "Number of topic", ylab = "NPMI", main = "Extrinsic topic conherence")
```
...
```{r}
LCP<- sapply(LDA_model, function(LDA_model) compute_c(LDA_model@beta, train, 15, "LCP"))
LCP<- data.frame(seq(10,100,10),LCP)
plot(LCP,type = "o", col= "blue",xlab = "Number of topic", ylab = "LCP", main = "Intrinsic topic conherence")
```
...
```{r}
#train_features <- sapply(LDA_model, function(x) x@gamma)
cluster<-list()
for(i in 1:10){
  x<-LDA_model[[i]]@gamma
  cluster[[i]]<-apply(x,1,which.max)
}
cluster[[2]][3]
```
...
purity <- function(clusters, labels){
  return(sum(apply(table(labels, clusters), 2, max)) / length(clusters))
}
purity(cluster[[10]],labels)
```

Finally, create a plot showing the purity for each of the ten k-values from the trained LDA models.
```{r}
p<-sapply(1:10, function(x) purity(cluster[[x]],labels))
purity_df<-data.frame(seq(10,100,10),p)
plot(purity_df,type = "o", col= "blue",xlab = "Number of topic", ylab = "Purity", main = "Purity analysis")
```
