...
def computeDelta(wt, X, Xi):
    ...
    for i in range(len(Xi)):
        numer = numer + (Xi.iloc[i][-1] * (np.exp(wt*similarity(X.iloc[:-1],Xi.iloc[i].iloc[:-1]))))    
        denom = denom + (np.exp(wt*similarity(X,Xi.loc[i]))) 
    emp = numer/denom
    return emp 
    pass
...
def similarity(a,b):
   ...
   numer = 0.0
   for i in range(M):
       numer = numer + ((a[i]-ma)*(b[i]-mb))
   
   s = numer/(M*a_std*b_std)  
   return s
